...
const ADMIN_ACTIVATION_CODE = process.env.ADMIN_ACTIVATION_CODE; // ya NO vive en el HTML
...
/** Trae el usuario actual desde el servidor de Supabase a partir del access_token.
 *  Importante: esto SIEMPRE devuelve el estado más reciente de la base de datos,
 *  a diferencia de leer los claims de un JWT viejo cacheado en el navegador. */
...
exports.handler = async (event) => {
  ...
  // ── ACCIÓN ESPECIAL: promover la propia cuenta a ADMIN ─────────
  // No exige ya ser ADMIN (es justo el mecanismo para llegar a serlo),
  // pero el código secreto solo se valida aquí, en el servidor.
  if (action === 'auth.promote') {
    if (!ADMIN_ACTIVATION_CODE) {
      return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: 'ADMIN_ACTIVATION_CODE no configurado en el servidor' }) };
    }
    if (!payload?.code || payload.code !== ADMIN_ACTIVATION_CODE) {
      return { statusCode: 403, headers: CORS, body: JSON.stringify({ error: 'Código de activación incorrecto' }) };
    }
    try {
      const r = await fetch(`${SUPABASE_URL}/auth/v1/admin/users/${user.id}`, {
        method: 'PUT',
        headers: svcHdrs,
        body: JSON.stringify({ app_metadata: { rol: 'ADMIN' } }),
      });
  ...
  // ── Todas las demás acciones requieren rol ADMIN ya asignado ──
  // Se lee de app_metadata (solo el servidor puede escribirlo, nunca el navegador).
